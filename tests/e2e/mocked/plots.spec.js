...
test.describe("Header — tips toggle", () => {
  ...
  test("both tips cards are visible by default", async ({ page }) => {
    await expect(
  ...
  test("clicking Show Tips restores both tips cards and updates the button", async ({
    page,
  }) => {
    await page.getByRole("button", { name: "Hide Tips" }).click();
...
test.describe("Time range synchronisation", () => {
  // 2026-01-01T12:00:00Z — getDayobsStartUTC("20260101")
  const FULL_START_MS = 1767268800000;
  // 2026-01-02T11:59:59Z — getDayobsEndUTC("20260101")
  const FULL_END_MS = 1767355199000;
  ...
  test("dragging on the timeline updates the time range input bar", async ({
    ...
    const range = FULL_END_MS - FULL_START_MS;
    const expectedStartValue = fmtMs(Math.round(FULL_START_MS + 0.2 * range));
    const expectedEndValue = fmtMs(Math.round(FULL_START_MS + 0.6 * range));

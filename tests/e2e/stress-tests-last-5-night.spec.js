...
function getFromAndToObsDays() {
  const toDate = new Date();
  const fromDate = new Date();
  fromDate.setDate(fromDate.getDate() - 5);
  const toDateObsDay = toDate.toISOString().split("T")[0].replaceAll("-", "");
  const fromDateObsDay = fromDate
    .toISOString()
    .split("T")[0]
    .replaceAll("-", "");
  return [fromDateObsDay, toDateObsDay];
}

const obsDays = getFromAndToObsDays();
const NIGHTLY_DIGEST_URL = `https://usdf-rsp-dev.slac.stanford.edu/nightlydigest/?startDayobs=${obsDays[0]}&endDayobs=${obsDays[1]}`;
...
const DATA_LOAD_TIMEOUT = 60000 * 2;

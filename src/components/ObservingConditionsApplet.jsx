...
const CustomTooltip = ({ active, payload, label, xDomain }) => {
  ...
  // Hide tooltip if data point is outside the visible X axis domain
  if (xDomain && (label < xDomain[0] || label > xDomain[1])) return null;
...
function ObservingConditionsApplet({
  ...
  const { xMin, xMax, xTicks } = useMemo(() => {
    ...
    const actualDataRange = chartData
      .map((d) => d.obs_start_dt)
      .filter(isValidNumber)
      // add twilight values to xVals to make sure they are included in the chart
      .concat(twilightValues);
    // Calculate min and max for x-axis data points
    const min = Math.min(...actualDataRange);
    const max = Math.max(...actualDataRange);

    // Now limit the selection (potentially) based off zoom/selection
    const xMin = Math.max(min, selectedMinMillis);
    const xMax = Math.min(max, selectedMaxMillis);
